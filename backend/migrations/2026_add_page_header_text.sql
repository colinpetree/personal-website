...
UPDATE site_config
SET blog_text = '<h1>' || COALESCE(blog_page_name, 'Blog') || '</h1>'
WHERE blog_text IS NULL;

UPDATE site_config
SET contact_text = '<h1>' || COALESCE(contact_page_name, 'Contact') || '</h1>'
WHERE contact_text IS NULL;

UPDATE site_config
SET ai_demo_text = '<h1>' || COALESCE(ai_demo_page_name, 'AI Implementations') || '</h1>'
    || '<p>A grid of small, focused demos showing off different Claude API capabilities.</p>'
WHERE ai_demo_text IS NULL;

UPDATE site_config
SET payment_text = '<h1>☕ ' || COALESCE(payment_page_name, 'Payment') || '</h1>'
