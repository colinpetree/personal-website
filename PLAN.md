...
### Not yet reflected anywhere in this plan (found during 2026-09-04 review, not yet written up)
...
- **Header block editor feature** (`HeaderNode` in `frontend/src/components/admin/editor/nodes.jsx`) — layouts, video/image backgrounds with LQIP blur-up, cinematic fullscreen navbar, shadow overlay, buttons. Four commits (`aa6671e`, `c100272`, `f6522d9`, `47d957b`), comparable in scope to the documented Link Group node but absent from §3d.
...
- **Site-wide search, blog categories, next/prev post navigation, self-hosted webfonts, Lexical font-family setting, Link Group node (drag-reorder + brand icons), auto-publish/auto-update pipeline** (beyond the initial §10 deploy pipeline).
- `CLAUDE.md`'s "registers 14 blueprints" line is stale — `backend/app.py` currently registers 22 (20 unconditional + 2 flag-gated).
- `notes/header-video-audio-bug.md` (gitignored, local-only) describes the video-audio-leak bug fixed by `f6522d9`, but its own status section is written as unconfirmed/pending user verification. The repo's most recent commit, `47d957b`, is a *further* fix ("export-time marker attribute") on top of that — meaning the notes file documents an intermediate, apparently-incomplete fix, not the final state. Worth updating or retiring that notes file to match `47d957b`.
