...
- [x] ~~No PostgreSQL backup strategy~~ — **implemented 2026-09-04, not yet run in production**: `deploy/scripts/backup.sh` (nightly via `personal-website-backup.timer`) does `pg_dump --format=custom` + a full `$DATA_DIR` tar (uploads, `.env`, `certbot_domain.txt`), rotates local copies (`BACKUP_RETENTION_DAYS`, default 14), and rsyncs to the Pi build box over SSH (`deploy/scripts/setup-backup-ssh.sh` provisions the key). Silent on success, emails via the existing watcher-alert path (`source=backup`) on any failure. Restore is a documented manual `pg_restore`/`tar` procedure, deliberately not scripted — see `deploy/BACKUP.md`. Needs a real first run against production (SSH key exchange with the Pi, timer enabled) before this can be called verified rather than just implemented.
