# Personal Website — Overview Plan (Status: 2026-08-08, reviewed 2026-09-14, 2026-09-15)
...
Surfaced during the deployment-infrastructure review (2026-08-08) but deliberately kept **out** of the deployment plan since they're application-code changes or operational additions independent of the build/deploy mechanics, and further code changes are expected before actual launch anyway. **Reviewed again 2026-09-04** — two items closed by the 2026-08-31 hardening pass (`c49d7d6`, "Harden backend/deploy for production: CORS, sessions, CSP, XSS, rate limiting"), which also isn't otherwise reflected anywhere in this doc. **Reviewed again 2026-09-14** (see new items below and the refreshed "Not yet reflected" list):

- [x] ~~`SESSION_COOKIE_SECURE` is not currently set~~ — **fixed 2026-08-31**: `app.config['SESSION_COOKIE_SECURE'] = is_production` (`backend/app.py:29`).
- ~~No infra- or app-level rate limiting on the AI Implementations endpoints~~ — **no longer a concern, per product decision 2026-09-04**: the AI demo access-approval system (see below) changed the intended usage model from "any signed-in visitor can run demos" to "the admin grants access individually to trusted people for live demoing." Realistically almost no one beyond the admin will hold access going forward, so per-call spend risk from an open/public user base doesn't apply. The general public sees the capabilities via recorded demo videos in blog posts instead of running them live. Rate limiting isn't worth building for this usage pattern.
- [x] ~~No PostgreSQL backup strategy~~ — **implemented 2026-09-04, redesigned 2026-09-09, restore drill + scripted restore completed 2026-09-15**: originally a push design (production tars `$DATA_DIR` nightly and rsyncs it to the Pi). Redesigned 2026-09-09 for two reasons found while planning the first real test: (1) the Pi is on university WiFi with no port-forwarding and a rotating public IP, so production can never reliably open a connection *to* it — direction flipped so the **Pi pulls from production** instead (production has a stable domain and SSH already open); (2) a fresh full tar of `uploads/` every night × `BACKUP_RETENTION_DAYS` (14) would have meant ~14x the media library's size in backup storage alone — at an anticipated ~10GB of uploads, ~140GB, which doesn't fit on either production's 16GB disk or the Pi's 50GB SD card. Now: `deploy/scripts/backup.sh` (nightly via `personal-website-backup.timer`) does `pg_dump --format=custom` + `restic backup` (uploads, the dump, `.env`, `certbot_domain.txt`) into a local restic repository, then `restic forget --keep-daily 14 --prune` — restic's content-addressed dedup means 14 days of retention costs close to 1x the media size, not 14x. `deploy/scripts/backup-pull.sh` (nightly via a Pi-side systemd `--user` timer, `personal-website-backup-pull.timer`, 05:30 UTC — past production's systemd unit's own 7200s timeout ceiling, not just the sum of its step timeouts) rsyncs that repo down to the Pi with a dedicated read-only-restricted key (`deploy/scripts/setup-backup-pull-pi.sh` provisions it; `deploy/scripts/setup-restic-repo.sh` replaces the old `setup-backup-ssh.sh` on the production side) and verifies a fresh snapshot landed via `restic snapshots`. Silent on success either side, emails via the existing watcher-alert path (`source=backup`) on any failure — the Pi has no local Mailgun access, so its half alerts via an HTTP POST to `/api/watcher-alert` (same mechanism `content-watch.sh` already used for gh-outage alerts). `deploy/scripts/backup-status.sh` (added 2026-09-14, commit `07a6341`) is a manual, read-only helper run on the Pi to eyeball restic snapshot list + mirror/repo size between the silent nightly runs. **2026-09-15**: the "verified in production" gap is now closed. `deploy/scripts/restore.sh` (runs on the target, restores DB/uploads/secrets from the latest local restic snapshot then installs the latest release) and `deploy/scripts/restore-remote.sh` (runs on the Pi, pushes its backup mirror to a from-scratch target over SSH since production can never reach the Pi, then drives `restore.sh` there) were built and run for real against `test633.org` end to end, including the Pi-driven push path, not just reasoned through. Real bugs were found and fixed during that drill: a repo-relocation step falsely treating a pushed copy of the box's own backup as a conflicting repository (fixed by comparing restic's own repository ID, not just the filesystem path); an f-string syntax incompatibility with the Pi's older Python; and a live-site safety gate requiring the operator to type the target's own domain (not a generic word) to confirm before overwriting a currently-healthy site, which cannot be bypassed by `--yes`. Documented in `deploy/BACKUP.md`.
- [x] ~~No monitoring/alerting for service crash-loops, disk usage, or Certbot renewal failures~~ — **implemented 2026-09-04, crash-loop detection fixed and confirmed live 2026-09-15**: `deploy/scripts/health-watch.sh` (every 15 min via `personal-website-healthwatch.timer`, auto-enabled by `install.sh` on first install) checks disk usage (`df` on `/` and `$DATA_DIR` against `BACKUP_DISK_ALERT_PERCENT`, default 85%) and live-certificate days-until-expiry against `CERT_ALERT_DAYS` (default 14), each debounced to one alert/day via a marker file. **2026-09-15**: a real DB-outage drill found the original crash-loop check (`systemctl is-failed personal-website`) could never fire, since `personal-website.service` had no restart burst limit and just retried forever rather than ever reaching `failed`, a real gap this project's own philosophy of "self-heal from brief outages" had silently defeated its own alerting. Fixed by keeping the unlimited-retry self-heal (`StartLimitIntervalSec=0`, explicit and documented) and replacing the crash-loop check with an active HTTP probe (3 attempts, up to ~70s) against the app itself, so a brief blip self-heals silently and only a sustained outage alerts. Also moved `personal-website-healthwatch.service` to run as root instead of `personalweb`, since reading another unit's journal for the alert's log tail would otherwise require granting the app's own service account (`personalweb`, also what gunicorn runs as) much broader journal-read access than it needs. All three checks route through the same watcher-alert/Mailgun path, including a database-independent fallback Mailgun path (`WATCHER_ALERT_FALLBACK_*`) for crash-loop alerts when Postgres itself is down. Confirmed working for real end to end (self-heal with zero manual restart, alert fires via the fallback path with an accurate message and populated log tail) via a live drill on `test633.org`, not just reasoned through. See `deploy/BACKUP.md`.
- No automated test suite in the repo (per `CLAUDE.md`) and no staging environment — **still open, reconfirmed 2026-09-14** — only vendored tests exist under `.venv`/`node_modules`; no pytest/vitest config in the project itself. Releases go straight from the Pi build to production install with no CI gate.
- [x] ~~Frontend API calls and Flask-CORS config haven't been explicitly re-verified for the production same-origin topology~~ — **resolved 2026-08-31**: CORS is now restricted to an explicit `FRONTEND_ORIGIN` allowlist (`backend/app.py:50-51`) instead of reflecting any origin. Remaining `localhost:5000` references (`frontend/src/lib/apiFetch.js`, `frontend/src/routes.ts`) are intentional server-side/build-time fallbacks gated behind `typeof window === 'undefined'`; browser requests use relative URLs. The same 2026-08-31 pass also added CSP headers, XSS/SVG-upload sanitization, bleach-based HTML sanitization for blog content, an open-redirect fix in the OAuth callback, and a one-time random admin seed password (replacing the static `admin@example.com`/`admin` default) — none of which were previously tracked in this plan.
- [x] **New 2026-09-14 — visitor IPs behind Cloudflare were being collapsed to one shared edge IP**, breaking per-IP rate limiting on contact/login forms. Fixed by `f095d0e` ("Recover real visitor IPs behind Cloudflare in nginx" — nginx `realip` module, `set_real_ip_from`/`real_ip_header CF-Connecting-IP`, with `install.sh` fetching Cloudflare's current IP ranges into the templated nginx config) and `796a025` ("Fix contact/login rate limiters collapsing all visitors into one IP").

### Not yet reflected anywhere in this plan (last refreshed 2026-09-14)

Substantial feature work has shipped since the 2026-08-08 status date that this document doesn't mention at all. Flagging for a future pass rather than writing full sections now:
- **Pages + Site Navigation feature — largely built, not a full fixed-page replacement.** `backend/models.py` now has a freeform `Page` model (title/slug/content_html/meta_description/draft-publish lifecycle, same shape as `BlogPost`) with CRUD in `backend/routes/admin_pages.py`, public resolution unified with blog-post slugs in `backend/routes/public_resolve.py`, and admin UI (`AdminPagesListPage.jsx`, `AdminPageEditorPage.jsx`). `AdminSiteNavigationPage.jsx` now edits a freeform `primary_navigation` link list + `site_title_link` on `SiteConfig`, replacing the old fixed nav-key concept. **About was fully retired** (`864610b`) — it's now just a regular `Page` row, no dedicated model/route or admin page. However `frontend/src/routes.ts` still hardcodes dedicated routes/admin pages for Home, Blog, Projects, Contact, AI Demo, and Payment — those did **not** get folded into the freeform Pages system, only About did. So §2/§3's "7 fixed pages" framing is now partially stale: 6 of the 7 are still fixed, About isn't, and an arbitrary number of extra freeform Pages can exist alongside them, routed through Site Navigation.
- **New admin-only Integrations page** (`frontend/src/pages/admin/AdminIntegrationsPage.jsx`, routed at `admin/integrations`) — Mailgun (API key/domain), Google Sign-In OAuth (client ID/secret/redirect URI, moved off the Users page per `c6c0213`), and Stripe keys all now live here instead of scattered across per-page config screens.
- **Header block editor feature** (`HeaderNode` in `frontend/src/components/admin/editor/nodes.jsx`) — 5 layouts (`regular`, `wide`, `full`, `fullscreen`, `split`), video/image backgrounds with LQIP blur-up, cinematic fullscreen navbar, shadow overlay, buttons. Continued through `be5ec77` (left-align wrapping, height scaling, mobile navbar fixes, 2026-09-14) on top of the four commits noted in the last review (`aa6671e`, `c100272`, `f6522d9`, `47d957b`); no pending/TODO markers found in the file — reads as a stable, comparable-in-scope feature to the documented Link Group node but still absent from §3d.
- **AI demo access-request/approval system** — `admin_ai_demo_links_bp` blueprint, `/api/ai-demo/request-access` + `/api/admin/ai-demo/access-requests`, `AiDemoAccessLink` model. Supersedes the simple sign-in gate §6 currently describes.
- **Site/blog analytics dashboards** — visitor/view/share tracking, Site vs. Blog dashboards, Payment Metrics page.
- **Site-wide search, blog categories, next/prev post navigation, self-hosted webfonts, Lexical font-family setting, Link Group node (drag-reorder + brand icons + GitHub icon/width options), auto-publish/auto-update pipeline** (beyond the initial §10 deploy pipeline).
- **Responsive admin dashboard** (`1235037`, "Make admin dashboard responsive and add mobile-only menus") — not previously tracked anywhere.
- `CLAUDE.md`'s "registers 14 blueprints" line is stale — `backend/app.py` currently registers **25** (up from the 22 noted at the last review).
- Release cadence has outpaced git tags: `VERSION` is now `0.6.5` and commit history shows Release v0.6.2 through v0.6.5, but `git tag` only goes up to `v0.6.1` — worth reconciling if tags are relied on for anything.
- [x] ~~`notes/header-video-audio-bug.md` (gitignored, local-only) status unclear~~ — **checked 2026-09-14**: the file already ends with `Status: RESOLVED`, documenting the real fix (`47d957b`, "export-time marker attribute" — a follow-up on top of the initial `f6522d9` mute-ordering fix that turned out insufficient on its own) and confirming no further audio-leak reports since. No action needed.
