...
function RecordingNodeComponent({ nodeKey, editor }) {
    ...
    try {
      ...
      if (!res.ok) {
        if (data.error === 'ffmpeg_not_found') {
          setSaveError('Error: Cannot convert audio to MP3. FFmpeg not found on the server.')
        } else {
          setSaveError(`Error: Failed to save recording.${data.detail ? ` ${data.detail}` : ''}`)
        }
        setPhase('recorded')
        return
      }
      ...
      setSaveError('Error: Failed to save recording.')
      setPhase('recorded')
    }
  }
  ...
  const ringClass = isSelected
    ...
    : isHovered
    ? 'ring-1 ring-blue-300 border-transparent'
    : 'border-gray-200'

  return (
    <div
      ref={containerRef}
      className="my-4 max-w-3xl mx-auto select-none"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
        ...
        {phase === 'idle' && (
          <Tooltip content="Start recording">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-gray-300 text-gray-600 hover:border-red-400 hover:text-red-600 hover:bg-red-50 transition-colors text-sm font-medium"
            >
        ...
        {phase === 'recording' && (
          <Tooltip content="Stop recording">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-red-400 text-red-600 bg-red-50 hover:bg-red-100 transition-colors text-sm font-medium"
            >
        ...
        {(phase === 'recorded' || phase === 'saving') && (
          <Tooltip content={isPlaying ? 'Pause' : 'Play'}>
            <button
              ...
              disabled={phase === 'saving'}
              className="p-2 rounded-md text-gray-600 hover:bg-gray-200 transition-colors disabled:opacity-40"
            >
        ...
        {(phase === 'recorded' || phase === 'saving') && (
          <Tooltip content="Delete recording">
            <button
              ...
              disabled={phase === 'saving'}
              className="ml-auto p-2 rounded-md text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors disabled:opacity-40"
            >
              <Trash2 size={18} />
            </button>
          </Tooltip>
        ...
        <div className="flex items-center gap-2 mt-3">
          ...
          <Tooltip content="Save recording">
            <button
              onClick={handleSave}
              disabled={phase === 'saving'}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-gray-300 text-gray-600 hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50 transition-colors text-sm font-medium disabled:opacity-40"
            >
              <Save size={14} />
              Save
            </button>
      ...
      {/* Error toast */}
      {saveError && (
        <div className="mt-3 flex items-center gap-2 px-3 py-2 bg-red-50 border border-red-200 rounded-md text-sm text-red-700">
      ...
      {showConfirm && (
        <div className="absolute inset-0 flex items-center justify-center bg-white/90 rounded-lg z-10">
          <div className="text-center p-4">
            <p className="text-sm text-gray-700 mb-4">Delete this recording?</p>
...
export class RecordingNode extends DecoratorNode {
  static getType() { return 'recording' }
  static clone(node) { return new RecordingNode(node.__key) }
  static importJSON() { return new RecordingNode() }
  exportJSON() { return { type: 'recording', version: 1 } }
  exportDOM() { return { element: null } }
  static importDOM() { return null }
  createDOM() { const s = document.createElement('span'); s.style.display = 'contents'; return s }
  updateDOM() { return false }
  isInline() { return false }
  decorate(editor) {
    return <RecordingNodeComponent nodeKey={this.getKey()} editor={editor} />
  }
}

export function $createRecordingNode() { return new RecordingNode() }
