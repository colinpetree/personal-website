...
// fullscreen-, full-width-, split-, or linear-layout HeaderNode (<header
// class="header-fullscreen">, <header class="header-full">, <header
// class="header-split">, or <header class="header-linear">, see nodes.jsx;
// "Full screen", "Full width", "Split", and "Linear" are the layouts meant to
...
// four layouts (drives the Navbar overlay above). isFullscreenHeader is true
// for "Full screen" always, and for "Linear" only once its height (which
// scales via `min(max(280px, 56.25vw), 100vh)`, see nodes.jsx) is actually
// pinned to the 100vh ceiling; at that point it visually fills the screen
// just like fullscreen does, so callers like FullscreenHeaderNav should show
// the same scroll-down hint. Full-width/split headers don't fill the
// viewport, so the hint would be misleading there.
export function useFullscreenHeaderNav(containerRef, contentKey) {
  ...
  useEffect(() => {
    ...
    // matching `min-width: 1536px` rule there.
    function update() {
      const linearFillsViewport = isLinear && (window.innerWidth >= 1536 || window.innerWidth * 0.5625 >= window.innerHeight)
      setState({ isOverlayHeader: isOverlay, isFullscreenHeader: isFullscreen || linearFillsViewport })
    }
    update()

    if (isLinear) window.addEventListener('resize', update)
    return () => {
      if (nav) nav.setOverlay(false)
      if (isLinear) window.removeEventListener('resize', update)
    }
