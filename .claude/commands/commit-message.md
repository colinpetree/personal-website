...
3. Write a concise commit message: one summary line (under 72 chars), followed by a blank line and a short list of the key changes using "- " prefixes. Always wrap the entire commit message in a markdown code block so it copies cleanly without bullet characters.
...
For simple changes, a single summary line is enough — skip the list if the change is self-explanatory. Always use a code block. Never use markdown bullet characters (•) — only "- " dash prefix.