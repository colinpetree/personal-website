...
if [ -z "${RESTIC_PASSWORD:-}" ]; then
    _alert "RESTIC_PASSWORD is not set in $DATA_DIR/.env — run deploy/scripts/setup-restic-repo.sh once."
    exit 1
fi
if [ ! -d "$RESTIC_REPO" ]; then
    _alert "No restic repository at $RESTIC_REPO — run deploy/scripts/setup-restic-repo.sh once."
    exit 1
fi
...
if ! timeout 1800 restic -r "$RESTIC_REPO" backup \
    ...
    _alert "restic backup failed or timed out after 30m — see journalctl -u personal-website-backup for details."
    exit 1
fi
...
if ! timeout 1800 restic -r "$RESTIC_REPO" forget --keep-daily "$RETENTION_DAYS" --prune; then
    _alert "restic forget/prune failed or timed out after 30m — local backup succeeded but old snapshots were NOT pruned. See journalctl -u personal-website-backup for details."
    exit 1
fi
